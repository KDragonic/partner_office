...
// Тут данные которые в данный запроса
current_search_data = {}
...
// Это все параметры фильтра, если они изменятся запустится новый фильтер
$(`.filter-select input[type='text'], .filter-select select, .filter-select input[type='checkbox'], *[name="sorting"]`).change(() => get_hotel_fillter())
...
// Перезапустить переменые фильтра (Когда выбераешь новые параметры и при генерации нового fillter_id)
function reset_fillter() {
	full_card_grupe = false
...
function collect_current_search_data() {
	current_search_data = {
	...
	if (buildings_list.length > 0) current_search_data["buildings"] = buildings_list
	if (rservices_list.length > 0) current_search_data["rservices"] = rservices_list
	if (hservices_list.length > 0) current_search_data["hservices"] = hservices_list
	if (additional_criterias_list.length > 0) current_search_data["additional_criterias"] = additional_criterias_list
	if($(`*[name="name_hotel"]`).val().length > 0) current_search_data["name_hotel"] = $(`*[name="name_hotel"]`).val()
	if($(`*[name="sorting"]`).val() != "not") current_search_data["sorting"] = $(`*[name="sorting"]`).val()
	current_search_data["count_card_max"] = count_card_max
	qp = getQueryParams()
	current_search_data = { ...current_search_data, ...qp };
	current_search_data["slug"] = slug
}
...
function get_hotel_fillter() {
	...
	collect_current_search_data()
	get_hotel_fillter_ajax();
}

// Запрос на получения отелей
function get_hotel_fillter_ajax() {
	...
	$.ajax({
		method: "GET",
		data: current_search_data,
		url: "/hotel/search/fillter/",
		beforeSend: function (request) {
			request.setRequestHeader("X-CSRFToken", getCookie('csrftoken'));
		},
		success: (request) => get_hotel_fillter_ajax_success(request)
	});
}

function get_differenceInDays() {
	let date1 = $("#date_1").val();
	let date2 = $("#date_2").val();

	var date1_p = date1.split(".");
	var date2_p = date2.split(".");


	// Преобразование значений в объекты даты
	let startDate = new Date(date1_p[2], date1_p[1], date1_p[0]);
	let endDate = new Date(date2_p[2], date2_p[1], date2_p[0]);

	var timeDiff = Math.abs(endDate.getTime() - startDate.getTime());
	differenceInDays = Math.ceil(timeDiff / (1000 * 3600 * 24));
}


// При успешном получение ответа от запроса
function get_hotel_fillter_ajax_success(request) {
	$("#next_card_hotel_loading").remove()

	// Проверить пришел запрос на текущий нужный ли запрос
...
// Создать карточку которую нажали на мини карте в списке карточек сверху
function gen_card_hotel_to_list(card, type = "def") {

	let text_under_price_buff = ""
	...
	if (differenceInDays > 1) {
		text_under_price_buff = `${card.min_price} ₽ за ${text_under_price}`
	}

	min_price_one = Math.ceil(card.min_price / differenceInDays)
	...
	if ($(window).width() > 1024) {
		...
		var elem = $(`
		<li class="card-hotel__item" data-card-type="${type}" data-hotel-id="${card.id}">
		...
		<div class="card-hotel__body" style="padding: 7px;">
			<div class="hotel-top" style="display: flex; flex-direction: column; justify-content: space-between; height: 100%;">
				...
				<div class="hotel-top__left" style="width: 100%;text-align: end;display: flex;margin-left: auto;">
						<span class="text" style="margin: -6px 0;">${text_under_price_buff}</span>
						<button class="hotel-top__button" style="width: 190px;">
							<a target="_blank"
								href="/hotel/${card.id}/${queryString}"
								style="color:inherit">${min_price_one} ₽ за ночь</a>
						</button>
					</div>
				</div>
			</div>
...
// Разместить карточки в HTML
function reset_hotel_card() {

	let card_is_missing = false

	for (let i = 0; i < hotels.length; i++) {
		let card = hotels[i];
		let id = card["id"];

		if ($(`#card-hotel .card-hotel__item[data-hotel-id=${id}]`).length == 0) {
			card_is_missing = true;
			break;
		}
	}

	if (!card_is_missing) return
	...
	if (hotels.length == 0 && final_request) {
	...
	$('#next_card-hotel').click(function () {
		current_search_data["count_card_max"] += 20
